...
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_doctor_requests_updated_at'
  ) THEN
    CREATE TRIGGER update_doctor_requests_updated_at BEFORE UPDATE ON doctor_requests FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
  END IF;
END $$;
